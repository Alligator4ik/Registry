...
class RegistryValue
{
  public:
    ...
    virtual StringT GetString() const { return StringT(); }
...
class Value : public RegistryValue
{
  ...
  public:
    ...
    {
        return (m_type == Registry::ValueType::INT32 || m_type == Registry::ValueType::INT64);
    }
...
class RegistryString final : public Value<ValueType::STRING, StringT>
{
  public:
    RegistryString(const StringT& value) : Value(value) {}

    StringT GetString() const override { return m_value; }
};
...
class RegistryKey
{
  ...
  public:
    ...
    ///	</param>
    /// <exception></exception>
    RegistryKey_ptr Open(const StringT& path, DesiredAccess access = DesiredAccess::Read)
    {
        if (path.empty())
        {
    ...
    RegistryKey_ptr CreateVolatile(const StringT& path, DesiredAccess access = DesiredAccess::Read)
    {
    ...
    RegistryKey_ptr Create(const StringT& path,
                           ...
                           CreateKeyOptions options = CreateKeyOptions::NonVolatile)
    {
        ...
        LSTATUS lStatus = RegCreateKeyEx(m_hKey,
                                         path.c_str(),
                                         dwReserved,
                                         lpClass,
    ...
    void Delete(const StringT& name)
    {
    ...
    bool HasKey(const StringT& path)
    {
        if (path.empty())
        {
            throw std::invalid_argument("Specified path to registry key cannot be empty");
    ...
    bool Exists(const StringT& path) { return HasKey(path); }

    bool HasValue(const StringT& name)
    {
        if (name.empty())
        {
    ...
    bool GetBoolean(const StringT& name)
    {
        DWORD dwType = 0;
        DWORD dwData = 0;
        ...
        LSTATUS lStatus =
            RegQueryValueEx(m_hKey, name.c_str(), lpReserver, &dwType, reinterpret_cast<LPBYTE>(&dwData), &cbData);
        if (lStatus != ERROR_SUCCESS)
        {
            auto ec = std::error_code(lStatus, std::system_category());
    ...
    bool GetBoolean() { return GetBoolean(""); }

    void SetBoolean(const StringT& name, bool value)
    {
        DWORD dwValue = value ? 1 : 0;
        DWORD cbData = sizeof(dwValue);

        LSTATUS lStatus =
            RegSetValueEx(m_hKey, name.c_str(), 0, REG_DWORD, reinterpret_cast<const LPBYTE>(&dwValue), cbData);
        if (lStatus != ERROR_SUCCESS)
        {
            auto ec = std::error_code(lStatus, std::system_category());
    ...
    void SetBoolean(bool value) { SetBoolean("", value); }

    long GetInt32(const StringT& name)
    {
        long lData = 0;
        DWORD cbData = sizeof(lData);

        LSTATUS lStatus =
            RegQueryValueEx(m_hKey, name.c_str(), nullptr, nullptr, reinterpret_cast<LPBYTE>(&lData), &cbData);
        if (lStatus != ERROR_SUCCESS)
        {
            auto ec = std::error_code(lStatus, std::system_category());
    ...
    long GetInt32() { return GetInt32(""); }

    unsigned long GetUInt32(const StringT& name) { return static_cast<unsigned long>(GetInt32(name)); }

    unsigned long GetUInt32() { return GetUInt32(""); }

    void SetInt32(const StringT& name, long value)
    {
        DWORD cbData = sizeof(value);

        LSTATUS lStatus =
            RegSetValueEx(m_hKey, name.c_str(), 0, REG_DWORD, reinterpret_cast<const LPBYTE>(&value), cbData);
        if (lStatus != ERROR_SUCCESS)
        {
            auto ec = std::error_code(lStatus, std::system_category());
    ...
    void SetInt32(long value) { return SetInt32("", value); }

    void SetUInt32(const StringT& name, unsigned long value) { return SetInt32(name, static_cast<long>(value)); }

    void SetUInt32(unsigned long value) { return SetUInt32("", value); }

    long long GetInt64(const StringT& name)
    {
        long long llData = 0;
        DWORD cbData = sizeof(llData);

        LSTATUS lStatus =
            RegQueryValueEx(m_hKey, name.c_str(), nullptr, nullptr, reinterpret_cast<LPBYTE>(&llData), &cbData);
        if (lStatus != ERROR_SUCCESS)
        {
            auto ec = std::error_code(lStatus, std::system_category());
    ...
    long long GetInt64() { return GetInt64(""); }

    unsigned long long GetUInt64(const StringT& name) { return static_cast<unsigned long long>(GetInt64(name)); }

    unsigned long long GetUInt64() { return static_cast<unsigned long long>(GetUInt64("")); }

    void SetInt64(const StringT& name, long long value)
    {
        DWORD cbData = sizeof(value);

        LSTATUS lStatus =
            RegSetValueEx(m_hKey, name.c_str(), 0, REG_QWORD, reinterpret_cast<const LPBYTE>(&value), cbData);
        if (lStatus != ERROR_SUCCESS)
        {
            auto ec = std::error_code(lStatus, std::system_category());
    ...
    void SetInt64(long long value) { return SetInt64("", value); }

    void SetUInt64(const StringT& name, unsigned long long value) { SetInt64(name, static_cast<long long>(value)); }

    void SetUInt64(unsigned long long value) { SetUInt64("", value); }

    StringT GetString(const StringT& name)
    {
        DWORD cbData = 0;
        DWORD dwType = 0;
        ...
        if (cbData)
        {
            ...
            lStatus =
                RegGetValue(m_hKey, nullptr, name.c_str(), dwFlags, &dwType, reinterpret_cast<LPBYTE>(data), &cbData);
    ...
    StringT GetString() { return GetString(""); }

    void SetString(const StringT& name, const StringT& value)
    {
        auto cbData = static_cast<DWORD>(value.length());

        LSTATUS lStatus = RegSetValueEx(
            m_hKey, name.c_str(), 0, REG_SZ, reinterpret_cast<const BYTE*>(value.c_str()), cbData * sizeof(TCHAR));
        if (lStatus != ERROR_SUCCESS)
        {
            auto ec = std::error_code(lStatus, std::system_category());
    ...
    void SetString(const StringT& value) { SetString("", value); }
    ...
    void SetExpandString(const StringT& name, const StringT& value)
    {
        auto cbData = static_cast<DWORD>(value.length());
    ...
    /// </summary>
    /// <param name="value">Value to be set</param>
    void SetExpandString(const StringT& value) { SetExpandString("", value); }
    ...
    void EnumerateSubKeys(const __Function& callback)
    {
        ...
        LSTATUS lStatus = RegQueryInfoKey(m_hKey,               // Key handle
        ...
        for (DWORD i = 0; i < dwSubKeys; ++i)
        {
            ...
            if (lStatus != ERROR_SUCCESS)
            {
                ...
                pex = std::make_exception_ptr(std::system_error(ec, "RegQueryInfoKey() failed"));

                break;
            }

            StringT subKeyName(pszName, dwLen);
    ...
    void Notify(bool watchSubtree = false,
                NotifyFilter notifyFilter = NotifyFilter::ChangeName | NotifyFilter::ChangeAttributes)
    {
        HANDLE hEvent = nullptr;
        BOOL fAsynchronous = FALSE;

        LSTATUS lStatus = RegNotifyChangeKeyValue(
            m_hKey, watchSubtree ? TRUE : FALSE, static_cast<DWORD>(notifyFilter), hEvent, fAsynchronous);
    ...
    /// <summary>
    ///	Notifies the caller about changes to the attributes or contents of a specified registry key asynchonously via
    /// provided event object.
    /// </summary>
    /// <param name="hEvent">
    ///	A handle to an event. Call to this function returns immediately and changes are reported by signaling this
    /// event.
    /// </param>
    /// <param name="watchSubtree">
    ///	If this parameter is true, the function reports changes in the specified key and its subkeys.
    ...
    /// Default is ChangeName and ChangeAttributes.
    /// </param>
    void NotifyAsync(HANDLE hEvent,
                                                 ...
                                                 NotifyFilter::ChangeLastSet)
    {
        ...
        if (lStatus != ERROR_SUCCESS)
        {
...
#if 0
			std::shared_ptr<BYTE> GetBinary(const StringT& name, size_t len)
			{
			...
			bool SetBinary(const StringT& name, const BYTE* pData, size_t len)
			{
				DWORD cbData = DWORD(len);
			...
			RegistryValue GetValue(const StringT& name)
			{
			...
			void SetValue(const StringT& valueName, const RegistryValue& value)
			{
...
#if 0
			std::vector<StringT> GetSubKeys()
			{
				std::vector<StringT> vecRet;
				DWORD    dwSubKeys = 0;
				...
				if (lStatus == ERROR_SUCCESS && dwSubKeys > 0)
				{
					...
					for (DWORD i = 0; i < dwSubKeys; i++)
					{
						...
						if (lStatus == ERROR_SUCCESS)
						{
							vecRet.push_back(StringT(pwszName, dwNameLen));
						}
					}
				}
